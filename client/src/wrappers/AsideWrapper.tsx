...
const AsideWrapper = (props: Props) => {
    ...
    if (props.authentificationToken) {
        return <>
            <div onClick={() => setIsDrawerOpen(prev => !prev)} className={classes.iconButton} >
                <div className={classes.hamburger} style={{ transform: !isDrawerOpen ? 'rotate(30deg)' : 'rotate(0deg)' }} />
                <div className={classes.hamburger} style={{ opacity: !isDrawerOpen ? 0 : 1 }} />
                <div className={classes.hamburger} style={{ transform: !isDrawerOpen ? 'rotate(-30deg)' : 'rotate(0deg)' }} />
            </div>
            <aside className={classes.aside} style={{ transform: !isDrawerOpen ? 'translateX(0px)' : `translateX(-${dimensions.asideWidth})`, opacity: !isDrawerOpen ? 1 : 0 }}>
                <Box className={classes.asideItemsPosition}>
                    <Box className={classes.routesPosition}>
                        {Routes}
                    </Box>
